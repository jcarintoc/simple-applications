...
// Protected feed route (must be before /:id route)
router.get("/feed", authMiddleware, (req, res) => postController.getFeed(req as AuthRequest, res));
...
router.post("/:id/like", authMiddleware, verifyCsrfToken, (req, res) =>
  postController.like(req as AuthRequest, res)
);
